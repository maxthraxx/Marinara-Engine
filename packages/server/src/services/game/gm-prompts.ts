...
export function buildGmFormatReminder(
  ...
  lines.push(
    `<output_format>`,
    `Think HARD about your response first. Remember your Core Responsibilities and the Prose Quality that is expected of you. Then respond, following the output format below.`,
    `VISUAL NOVEL STYLE (MANDATORY):`,
    `Every line of your output must use one of these formats:`,
    ` 1. Narration:`,
    `   Plain text, 1-4 sentences per beat. Beats separated by blank lines. Do not start sentences with "somewhere" or "outside", these are BANNED!`,
    ` 2. Dialogue:`,
    `   [Name] [expression]: "Spoken text in double quotes."`,
    ...
    `Treat player inputs as attempts to perform in-world actions or engage in dialogue. Do not repeat the player's committed actions or words; proceed directly to the consequences, optionally straightening out the results. For example, if the player shouts "let me out" to a guard while bound and gagged, you should narrate something similar to, "At least, that's what you thought you articulated well, but it came out as a muffled 'mfm mmm mfm' instead." If the player is drunk while talking to someone, they might be told they are slurring too much to be understood. If they say they lifted a rock that their strength levels would certainly not allow them to lift, you can mention that the rock actually barely budged, and their backs will suffer for the attempt later on.`,
    `NEVER use [${ctx.playerName ?? "Player"}] [expression]: format for the player character.`,
    `Always narrate the player's speech in second-person indirect style:`,
  ...
  if (hasParty) {
    lines.push(
      ``,
      `PARTY DIALOGUE:`,
      `YOU also play the party members (${partyNames.join(", ")}). Interleave their dialogue and reactions naturally within your narration.`,
      ``,
      ...
      ``,
      `Party dialogue uses a DIFFERENT format from regular narration/dialogue lines. These are parsed separately by the UI:`,
      `  [Name] [main] [expression]: "Spoken dialogue." — Primary party dialogue. Displayed in the VN dialogue box with avatar and expression.`,
      `  [Name] [side] [expression]: "Quick remark." — Flavor, comedy, banter, interjections, butting in. Shown as floating box above the active dialogue.`,
      `  [Name] [thought] [expression]: Internal monologue... — Private thoughts. Styled differently in the VN box.`,
      `  [Name] [whisper:${ctx.playerName ?? "Player"}] [expression]: "Whispered aside." — Quiet aside to a specific character.`,
      ``,
      `Physical actions (gestures, movement, combat moves) should be written as normal narration lines between dialogue tags.`,
      ``,
      `Expression tags are MANDATORY: [happy], [smirk], [angry], [sad], [neutral], [surprised], [worried], [amused], [annoyed], [thinking], etc.`,
      ``,
      `INTERLEAVING RULES:`,
      `- Weave party lines into your narration naturally. A party member can react between narration beats or after NPC dialogue.`,
      `- Not every party member needs to speak every turn. Only those who would naturally react.`,
      `- Party members can talk to each other and to the player.`,
      `- Keep party moments proportional to the scene. Don't let party chatter overshadow a dramatic GM narrative beat.`,
    );
    if (ctx.talkToParty) {
      lines.push(
        ``,
